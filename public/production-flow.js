...
(function () {
  ...
  const workflowMaterial = (material) => {
    ...
    const purchaseBatch = material.batchNumber || material.lotNo;
    return `${Number(material.quantity || 0).toLocaleString()}${item?.unit ? ` ${item.unit}` : ''} ${material.name}${purchaseBatch ? ` · Purchase batch ${purchaseBatch}` : ''}`;
  };
