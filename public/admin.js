...
$('#add-person').onclick=()=>adminModal('person');$('#add-item').onclick=()=>adminModal('item');$('#record-form').addEventListener('submit',async e=>{let type=e.currentTarget.dataset.type;if(type!=='person'&&type!=='item')return;e.stopImmediatePropagation();e.preventDefault();let v=formData(e.currentTarget);adminData();if(type==='person'){if(v.type==='User'){let response=await fetch('/api/access/users',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:v.email})}),result=await response.json().catch(()=>({}));if(!response.ok){alert(result.error||'Cloudflare Access could not add this user.');return}v.accessPolicyId=result.accessPolicyId;v.accessStatus='Active'}data.people.unshift({id:id(),...v});if(v.role&&!data.roles.includes(v.role))data.roles.push(v.role)}else{data.items.unshift({id:id(),...v});if(v.category&&!data.categories.includes(v.category))data.categories.push(v.category);if(v.unit&&!data.units.includes(v.unit))data.units.push(v.unit)}save();$('#record-dialog').close();render()},true);render();
