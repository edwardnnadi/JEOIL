...
const dieselStandard={name:'Diesel quality standard',parameters:[{key:'density15',label:'Density at 15°C',operator:'≥',limit:'820',unit:'kg/m³'},{key:'flashPoint',label:'Flash point',operator:'≥',limit:'52',unit:'°C'},{key:'sulphur',label:'Sulphur content',operator:'≤',limit:'50',unit:'ppm'},{key:'waterSediment',label:'Water and sediment',operator:'≤',limit:'0.05',unit:'% vol'},{key:'cetaneIndex',label:'Cetane index',operator:'≥',limit:'45',unit:''}]};
const defaultItems=[{id:201,name:'Peanut kernels',category:'NUTS',unit:'kg'},{id:202,name:'Caustic soda',category:'Chemicals',unit:'kg'},{id:203,name:'Bleaching earth',category:'Chemicals',unit:'kg'},{id:204,name:'Diesel',category:'Petrol and Diesel',unit:'L',qualityStandard:dieselStandard},{id:205,name:'Charcoal',category:'Petrol and Diesel',unit:'bags'},{id:206,name:'Firewood',category:'Firewood',unit:'stacks'}];
const defaultCategories=['NUTS','Chemicals','Petrol and Diesel','Packaging','Maintenance','Other','Firewood','Storage Drums','Storage Sacks','Lab Equipment','Factory Floor Equipment'];
const legacyStarterCategories=['Peanut kernels','Chemicals','Fuel & energy','Packaging','Maintenance','Other'];
...
function adminData(){
  ...
  // Align installations created with the former local starter list to the
  // centrally maintained Purchase categories.  Only that exact legacy list
  // is replaced, so any categories an administrator has added stay intact.
  let categoriesAligned=false;
  if(!data.purchaseCategoryAlignment20260912&&data.categories.length===legacyStarterCategories.length&&data.categories.every(category=>legacyStarterCategories.includes(category))){
    const categoryMap={'Peanut kernels':'NUTS','Fuel & energy':'Petrol and Diesel'};
    const mappedCategory=(category,itemName)=>itemName==='Firewood'?'Firewood':(categoryMap[category]||category);
    data.categories=[...defaultCategories];
    data.items.forEach(item=>item.category=mappedCategory(item.category,item.name));
    data.purchases.forEach(purchase=>purchase.category=mappedCategory(purchase.category,purchase.item));
    data.stock.forEach(item=>item.category=mappedCategory(item.category,item.name));
    if(data.categoryStandards?.['Peanut kernels']&&!data.categoryStandards.NUTS)data.categoryStandards.NUTS=data.categoryStandards['Peanut kernels'];
    if(data.categoryStandards)delete data.categoryStandards['Peanut kernels'];
    categoriesAligned=true;
  }
  data.purchaseCategoryAlignment20260912=true;
  data.categoryStandards??={};
  if(!data.categoryStandards.NUTS) data.categoryStandards.NUTS=JSON.parse(JSON.stringify(peanutStandard));
  data.units??=[...defaultUnits];
  data.people.forEach(p=>{if(p.role&&!data.roles.includes(p.role))data.roles.push(p.role)});
  data.items.forEach(i=>{
    if(i.name==='Diesel'&&!i.qualityStandard)i.qualityStandard=JSON.parse(JSON.stringify(dieselStandard));
  ...
  // Record each item's receiving quality policy once, so it can be reviewed
  // and changed in Administration rather than inferred on every screen.
  if(window.StockLedger?.applyQualityPolicyDefaults(data))categoriesAligned=true;
  if(categoriesAligned) save();
}
function renderAdmin(){adminData();$('#people-table').innerHTML=data.people.map(p=>`<tr><td><strong>${p.name}</strong></td><td><span class="category">${p.type}</span></td><td>${p.role}</td><td>${p.email||p.phone||'—'}</td></tr>`).join('')||'<tr><td colspan="4">No people yet.</td></tr>';$('#items-table').innerHTML=data.items.map(i=>`<tr><td><strong>${i.name}</strong>${i.description?`<div class="item-note">${i.description}</div>`:''}<div class="item-note">${window.StockLedger?.requiresQualityCheck(i.name)?'Quality check required at receiving':'No quality check — accepted on receipt'}</div></td><td><span class="category">${i.category}</span></td><td>${i.unit}</td></tr>`).join('')||'<tr><td colspan="3">No purchase items yet.</td></tr>'}
...
openModal=(type,pid)=>{
  ...
  let form=$('#record-form'),oldCategory=form.elements.category,item=form.elements.namedItem('item'),unit=form.elements.unit;
...
const canDeleteRecords=()=>currentOperator?.()?.role==='Administrator';
...
openModal=(type,pid)=>{
  ...
  let form=$('#record-form'),item=form.elements.namedItem('item'),category=form.elements.category,unit=form.elements.unit,description=form.elements.itemDescription,qty=form.elements.qty,price=form.elements.unitPrice,total=form.elements.cost,speciesHost=document.createElement('div');speciesHost.className='field';item.closest('.field').insertAdjacentElement('afterend',speciesHost);
  function filterItemsByCategory(preferredItem=''){let available=data.items.filter(entry=>entry.category===category.value);item.innerHTML=available.length?available.map(entry=>`<option value="${entry.name}">${entry.name}</option>`).join(''):'<option value="">No purchase items in this category</option>';item.disabled=!available.length;item.value=available.some(entry=>entry.name===preferredItem)?preferredItem:(available[0]?.name||'')}
  ...
  item.onchange=()=>{sync(true);syncSpecies();syncQualityFields()};category.onchange=()=>{filterItemsByCategory(item.value);sync(true);syncSpecies();syncQualityFields()};qty.oninput=sync;price.oninput=sync;filterItemsByCategory(item.value);sync(true);syncSpecies();syncQualityFields();form.dataset.type='purchase-enhanced';$('#record-dialog').showModal();
};
