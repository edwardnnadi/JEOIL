...
function buildGoodsInwardsWizard(form,receipt){
  ...
  const show=next=>{step=next;if(step===groups.length-1&&typeof receivingApplyGate==='function')receivingApplyGate(form);panels.forEach((panel,index)=>panel.hidden=index!==step);progress.querySelectorAll('span').forEach((item,index)=>item.classList.toggle('active',index===step));controls.querySelector('.goods-wizard-back').hidden=step===0;controls.querySelector('.goods-wizard-next').hidden=step===groups.length-1;save.hidden=step!==groups.length-1;if(step===groups.length-1)save.textContent='Finish receipt';};
