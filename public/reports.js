...
(() => {
  ...
  const allAreas = ['Purchases', 'Suppliers', 'Goods Inwards', 'Production', 'Yield & Efficiency', 'Stock', 'Traceability', 'Custom Reports'];
  ...
  function purchaseRows() {
    return (data.purchases || []).map(purchase => ({
      date: dateValue(purchase.purchasedDate || purchase.date || purchase.createdDate), purchaseId: purchase.purchaseId || purchase.id, supplier: purchase.supplier || '', item: purchase.item || '', itemDescription: purchase.itemDescription || '', category: purchase.category || '', status: purchase.status || purchase.stage || '', lotNo: purchase.lotNo || '', unit: purchase.unit || '', purchasedQuantity: number(purchase.qty || purchase.quantity), purchasePrice: number(purchase.unitPrice || (number(purchase.cost) / (number(purchase.qty || purchase.quantity) || 1))), totalValue: number(purchase.cost || purchase.total), _record: purchase,
    }));
  }
  function goodsRows() {
  ...
  const sourceRows = source => ({ purchases: purchaseRows, suppliers: supplierRows, goodsInwards: goodsRows, production: productionRows, movements: movementRows, stock: stockRows, traceability: traceabilityRows }[source] || (() => []))();
  ...
  function renderReport() {
    ...
    const totals = Object.fromEntries(definition.columns.filter(measures.has).map(field => [field, rows.reduce((total, row) => total + number(row[field]), 0)]));
