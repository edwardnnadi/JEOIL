...
function buildPurchaseWizard(form){
  ...
  const show=next=>{step=next;const isReview=step===groups.length-1;panels.forEach((panel,index)=>panel.hidden=index!==step);progress.querySelectorAll('span').forEach((item,index)=>item.classList.toggle('active',index===step));controls.hidden=isReview;controls.querySelector('.wizard-back').hidden=step===0;save.hidden=!isReview;modalActions.hidden=!isReview;};
...
wizardStyle.textContent='.wizard-progress{display:flex;gap:8px;flex-wrap:wrap;margin:0 0 18px}.wizard-progress span{color:#887c68;font-size:12px;display:flex;gap:6px;align-items:center}.wizard-progress b{display:grid;place-items:center;flex:0 0 22px;width:22px;height:22px;border-radius:50%;background:#eee5d5;color:#5c513d}.wizard-progress span.active{color:#1e1b16;font-weight:700}.wizard-progress span.active b{background:#c89b3c;color:#16120a}.wizard-step header{margin-bottom:14px}.wizard-step h3{margin:0;color:#201b13}.wizard-step p{margin:4px 0 0;color:#776b58;font-size:13px}.wizard-controls{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-top:18px}.wizard-forward{display:flex;gap:10px}.wizard-step[hidden],.wizard-controls[hidden],.modal-actions[hidden]{display:none!important}#record-dialog{width:min(760px,calc(100vw - 24px));max-height:calc(100dvh - 24px);overflow:hidden}#record-form{max-height:calc(100dvh - 24px);min-width:0;overflow:auto;overscroll-behavior:contain}.purchase-wizard,.purchase-wizard .form-grid,.purchase-wizard .field{min-width:0}.purchase-wizard .form-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.purchase-wizard .field input,.purchase-wizard .field select,.purchase-wizard .field textarea{width:100%;min-width:0}.purchase-wizard input[name="purchaseQcStatus"][readonly]{background:#edf0ed;color:#6e756d;border-color:#d3d8d1;cursor:not-allowed}.purchase-wizard .item-note{overflow-wrap:anywhere}.purchase-wizard .purchase-stage-field{padding:13px 14px;border:1px solid #ded6b3;border-radius:8px;background:#fbf8ed;gap:7px}.purchase-wizard .purchase-stage-field label{color:#5c513d}.purchase-wizard .purchase-stage-field select{background:#fff;border-color:#c8b978;font-weight:700}.purchase-wizard .purchase-stage-field .item-note{margin:0;color:#75694e}.purchase-wizard .purchase-money-field input{text-align:right;font-variant-numeric:tabular-nums}@media(max-width:760px){#record-form{padding:18px}.modal-head{margin-bottom:16px}.modal-head h2{font-size:21px;line-height:1.15}.purchase-wizard .form-grid{grid-template-columns:minmax(0,1fr)}.purchase-wizard .wizard-progress{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px 12px}.purchase-wizard .wizard-progress span{font-size:11px;align-items:flex-start}.purchase-wizard .wizard-controls{position:sticky;bottom:-18px;background:#fff;padding:12px 0 2px;margin-top:16px}.wizard-forward{margin-left:auto}}';
