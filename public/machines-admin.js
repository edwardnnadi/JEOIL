...
(() => {
  ...
  const ensureMachines = () => {
    ...
    kurmaExpellers.forEach((machine) => {
      ...
      data.machines.push({ id: id(), ...machine, model: '15-ton expeller', manufacturerRating: { capacity: '', input: '', output: '' }, serialNumber: '', lastServiceDate: '', hoursSinceService: 0, serviceIntervalHours: 0, serviceIntervalDays: 0, notes: '' });
      added = true;
    });
    if (added) save();
  };
  ...
  grid.insertAdjacentHTML('beforeend', '<section class="panel table-panel admin-section-panel" id="machines-panel"><div class="panel-head"><div><h3>Machines</h3><p>Production equipment, manufacturer ratings, and preventive-maintenance settings.</p></div><button class="primary" id="add-machine">+ Add machine</button></div><table><thead><tr><th>Machine</th><th>Type</th><th>Manufacturer rating</th><th>Last service</th><th>Hour meter</th><th>Service status</th><th></th></tr></thead><tbody id="machines-table"></tbody></table></section>');

  const openMachineModal = (index) => {
    ensureMachines();
    const machine = index === undefined ? { type: 'Goyum Round Kettle Oil Expeller', manufacturer: 'Goyum', manufacturerRating: { capacity: '', input: '', output: '' }, lastServiceDate: today(), hoursSinceService: 0, serviceIntervalHours: 500, serviceIntervalDays: 180 } : data.machines[index];
    machine.manufacturerRating ??= { capacity: '', input: '', output: '' };
  ...
  const renderMachines = () => {
    ...
    table.innerHTML = data.machines.map((machine, index) => {
      ...
      const rating = machine.manufacturerRating || {};
      return `<tr><td><strong>${esc(machine.name)}</strong><div class="item-note">${esc(machine.manufacturer || '—')}${machine.model ? ` · ${esc(machine.model)}` : ''}${machine.serialNumber ? ` · ${esc(machine.serialNumber)}` : ''}</div></td><td>${esc(machine.type)}</td><td><div>Capacity: ${esc(rating.capacity || '—')}</div><div class="item-note">Input: ${esc(rating.input || '—')} · Output: ${esc(rating.output || '—')}</div></td><td>${machine.lastServiceDate ? date(machine.lastServiceDate) : 'Not recorded'}</td><td>${(Number(machine.hoursSinceService) || 0).toLocaleString()} h<div class="item-note">Interval: ${machine.serviceIntervalHours || '—'} h / ${machine.serviceIntervalDays || '—'} days</div></td><td><span class="badge ${state.tone}">${state.label}</span><div class="item-note">${state.detail}</div></td><td><button class="text-btn edit-machine" data-machine-index="${index}">Configure</button></td></tr>`;
    }).join('') || '<tr><td colspan="7">No machines configured yet. Add a Goyum or Kurma expeller to begin maintenance tracking.</td></tr>';
