...
function traceabilityData(){
  ...
  // A purchase owns its batch number from the moment it is raised.  The value
  // is deliberately stable: goods inwards, QC, warehouse movements and
  // production must all be able to point back to the same purchase batch.
  data.purchases.forEach(purchase=>{
    purchase.attachments ??= [];
    // Avoid calling lotReference here: it initializes this same state helper.
    purchase.batchNumber ??= purchase.lotNo || (purchase.purchaseId ? `${data.lotNoConfig.prefix}${String(purchase.purchaseId).replace(/[^A-Za-z0-9]/g,'').slice(-data.lotNoConfig.padding).padStart(data.lotNoConfig.padding,'0')}${data.lotNoConfig.suffix}` : '');
    purchase.lotNo ??= purchase.batchNumber;
  });
}
...
function traceabilityFields(purchase={}, isNew=false){
  ...
  return `<div class="form-grid purchase-traceability"><div class="field"><label>Purchase ID</label><input value="${escapeValue(reference)}" readonly><div class="item-note">Generated automatically from Admin settings.</div></div><div class="field"><label>Batch number</label><input name="batchNumber" value="${escapeValue(batchNumber)}" readonly><input name="lotNo" value="${escapeValue(batchNumber)}" type="hidden"><div class="item-note">Assigned when the purchase is created and retained throughout receiving, QC and stock movements.</div></div><div class="field"><label>Origin state</label><select name="originState" id="origin-state">${stateOptions(purchase.originState||'')}</select></div><div class="field"><label>Origin LGA</label><select name="originLga" id="origin-lga">${lgaOptions(purchase.originState||'',purchase.originLga||'')}</select></div><div class="field"><label>Collection site / community</label><input name="collectionSite" value="${escapeValue(purchase.collectionSite||'')}" placeholder="Village, market or collection point"></div><div class="field"><label>Origin code</label><input name="originCode" value="${escapeValue(purchase.originCode||'')}" maxlength="6" pattern="[A-Za-z]{2,6}" placeholder="e.g. Tar"><div class="item-note">Recorded as additional source traceability for this batch.</div></div><div class="field full"><label>Supplier receipt ID</label><input name="supplierReceiptId" value="${escapeValue(purchase.supplierReceiptId||'')}" placeholder="Receipt / invoice ID supplied by the supplier"></div>${purchaseQualityFields(purchase.purchaseQuality||assessmentFor(purchase)||{},!isNew,!isNew,isNew,purchase.item,purchase.category)}<div class="field full"><label>Attachments</label><input name="attachmentsInput" type="file" multiple accept="image/*,.pdf,.doc,.docx,.xls,.xlsx,.csv,.txt"><div class="item-note">Attach up to 5 pictures or documents, 1 MB each. Attachments are stored with this purchase.</div><div class="attachment-list"></div></div></div>`;
}
