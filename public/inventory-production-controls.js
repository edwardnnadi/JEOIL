...
(function () {
  ...
  async function endRun(form) {
    ...
    Object.assign(run,{status:'COMPLETED',endedAt,endedAtRecorded:now.toISOString(),timeCorrect:correct,warehouseId:outputWarehouse.id,warehouseName:outputWarehouse.name,actualRating:{capacity:form.elements.actualCapacity.value.trim(),input:form.elements.actualInput.value.trim(),output:form.elements.actualOutput.value.trim()},outputs,returnedMaterials,adjustment:form.elements.adjustment.value}); data.activeProductionRuns=data.activeProductionRuns.filter(entry=>String(entry.id)!==String(run.id)); data.production.unshift(run);
